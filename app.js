...
function load() {
    try {
        const raw = localStorage.getItem('ps99_taphero_players_v1');
...
function renderLeaderboard() {
    ...
    if (!list.length) {
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--text-muted)">
          ${state.mode === 'search' ? 'No players matched your search.' : 'No data yet — waiting for first snapshot from GitHub Actions.'}
        </td></tr>`;
        return;
    }
...
function renderPlayerDetail(userId) {
    ...
    // Build point history from all snapshots
