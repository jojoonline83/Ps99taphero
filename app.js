...
function save() {
    try { localStorage.setItem('ps99_taphero_v1', JSON.stringify(state)); } catch (_) {}
}

function load() {
    try {
        const raw = localStorage.getItem('ps99_taphero_v1');
...
function renderLeaderboard() {
    ...
    if (!list.length) {
        tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;padding:40px;color:var(--text-muted)">
          ${state.mode === 'search' ? 'No leagues matched your search.' : 'No data yet — waiting for first snapshot from GitHub Actions.'}
        </td></tr>`;
        return;
    }
...
function renderLeagueDetail() {
    ...
    document.getElementById('league-detail-sub').textContent = 'Tap Hero Competition';
...
function renderDeltaStat(elId, detail, windowMs, toleranceMs) {
    ...
    if (!snap) { el.textContent = '—'; return null; }
    const entry = findLeagueInSnapshot(snap, detail.ID, detail.Name);
    if (!entry) { el.textContent = '—'; return null; }
...
function clearSearch() {
    ...
    if (state.mode === 'search') { state.mode = 'top'; renderLeaderboard(); }
}
...
async function buildLiveDetail(raw) {
    ...
    return { ID: raw.ID, Name: raw.Name, Points: raw.Points, MemberCapacity: raw.MemberCapacity, Level: raw.Level, Created: raw.Created, roster };
}
